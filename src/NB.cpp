...
NB_NetworkStatus_t NB::begin(const char *pin, const char *apn, const char *username, const char *password, bool restart,
                             bool synchronous, bool disableErrors) {
...
bool NB::shutdown() {
    if (_state == NB_READY) {
        _modem.send("AT+CPWROFF");
        _modem.waitForResponse(40000);
    }
    _modem.end();
    _state = NB_OFF;
    return true;
...
int NB::ready() {
    ...
    switch (_readyState) {
        ...
        case READY_STATE_CHECK_SIM: {
            _modem.setResponseDataStorage(&_response);
            _modem.send("AT+CPIN?");
        ...
        case READY_STATE_SET_APN: {
            _modem.sendf("AT+CGDCONT=1,\"IP\",\"%s\"", _apn);
        ...
        case READY_STATE_SET_APN_AUTH: {
            if (strlen(_username) > 0 || strlen(_password) > 0) {
                // CHAP
                _modem.sendf("AT+UAUTHREQ=1,2,\"%s\",\"%s\"", _password, _username);
            } else {
                // no auth
                _modem.sendf("AT+UAUTHREQ=1,%d", 0);
            }
